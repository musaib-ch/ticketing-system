...
from .forms import UserRoleForm, TeamForm, AddUserForm, UserProfileForm
...
def edit_team(request, team_id):
    ...
    if request.method == 'POST':
        ...
        # Simple SLA/Close updates
            ...
            try:
                sla_days = max(0, int(request.POST.get('sla_days', team.sla_days)))
                auto_close_days = max(0, int(request.POST.get('auto_close_days', team.auto_close_days)))
            except (TypeError, ValueError):
                messages.error(request, "SLA and auto-close values must be whole numbers.")
                return redirect('edit_team', team_id=team.id)
            team.sla_days = sla_days
            team.auto_close_days = auto_close_days
            team.save()
        ...
        # Upload Resource File
            ...
            if file:
                ...
                from tickets.utils import validate_uploaded_file
                try:
