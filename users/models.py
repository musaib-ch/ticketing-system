...
class Branding(models.Model):
    ...
    background_image = models.ImageField(upload_to='branding/', null=True, blank=True)
...
class SmtpSettings(models.Model):
    """Singleton that lets superadmins configure SMTP from the portal.

    Values stored here override the environment variables at send-time.
    The record with pk=1 is the only one ever used.
    """
    host = models.CharField(max_length=255, blank=True, help_text='SMTP server hostname, e.g. smtp.gmail.com')
    port = models.PositiveIntegerField(default=587)
    username = models.CharField(max_length=255, blank=True)
    password = models.CharField(max_length=255, blank=True)
    use_tls = models.BooleanField(default=True)
    from_email = models.EmailField(blank=True, help_text='Sender address shown to recipients')
    ...
    def __str__(self):
        return f'SMTP: {self.host}:{self.port} ({"enabled" if self.enabled else "disabled"})'
