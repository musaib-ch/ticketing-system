...
# Django's static()/staticfiles helper for MEDIA_URL (see config/urls.py) is a
# no-op when DEBUG=False - Django never serves local media files in
# production. That means ticket attachments, KB files, and branding images
# are only reachable in production via S3-compatible storage. Fail fast with
# a clear error instead of silently deploying a build where every
# attachment/logo link 404s.
if not DEBUG and not os.getenv('AWS_STORAGE_BUCKET_NAME'):
    raise RuntimeError(
        'AWS_STORAGE_BUCKET_NAME (and the other AWS_S3_* variables) must be set '
        'when DJANGO_DEBUG=False. Django does not serve local media files in '
        'production, so ticket attachments, knowledge-base files, and branding '
        'images need S3-compatible storage (e.g. Supabase Storage) - see RENDER_DEPLOY.md.'
    )
...
if not DEBUG:
    # Render (and most PaaS/CDN fronts) terminates TLS at its edge and forwards
    # plain HTTP to this app, with the original scheme passed in
    # X-Forwarded-Proto. Setting SECURE_SSL_REDIRECT=True here would cause an
    # infinite redirect loop, because Django would always see the incoming
    # request as HTTP unless it trusts that header (configured below) - and
    # even then, Render's own edge already enforces HTTPS for end users, so
    # this redirect is opt-in rather than assumed.
    ...
    # Trust the X-Forwarded-Proto header set by Render's edge proxy so Django
    # knows the original request was HTTPS.
    SECURE_PROXY_SSL_HEADER = ('HTTP_X_FORWARDED_PROTO', 'https')

    # Use the Host header forwarded by Render's proxy instead of the raw server host.
