...
def send_ticket_notification(ticket, is_new=True):
    """Send an email to functional reps when a ticket is assigned to their team."""
    if not ticket.team:
        return
...
def send_reply_to_requester(ticket_message):
    """Send an email to the employee when a functional rep replies."""
    ...
    message += "---\nReply to this email to add a comment to the ticket."
    try:
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [requester_email], fail_silently=True)
    except Exception as e:
        logger.error(f"Failed to send email: {e}")
