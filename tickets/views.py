...
def dashboard(request):
    ...
    if sort_by == 'ticket':
        order_field = 'ticket_number'
    elif sort_by == 'requester':
        order_field = 'requester__first_name'
    ...
    if order_field:
        ...
        tickets = tickets.order_by(status_order, '-created_at')
        
    # 3. Context & Counts
    context = {}
    context['tickets'] = tickets[:15]
...
def queue(request):
    ...
    return render(request, 'queue.html', {'tickets': tickets, 'page_title': page_title})
...
def department_queue(request):
    ...
    return render(request, 'queue.html', {'tickets': tickets, 'page_title': 'Department Queries'})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.FILES.get('attachment'):
                Attachment.objects.create(ticket=ticket, file=request.FILES['attachment'])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.errors:
                ...
                if request.FILES.get('attachment'):
                    Attachment.objects.create(ticket=ticket, message=msg, file=request.FILES['attachment'])
...
def export_tickets_csv(request):
    ...
    for t in tickets:
        ...
        writer.writerow([
            ...
            t.assigned_agent.get_full_name() or t.assigned_agent.username if t.assigned_agent else "Unassigned",
